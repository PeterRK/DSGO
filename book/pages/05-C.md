...
```
　　B+树由索引节点和存储节点组成，是一种完美平衡的多叉树。其中存储节点作为树的叶节点同时，还构成有序块链表。索引节点中的索引值为其对应子节点内容（索引或负载）的最大值。
```
		   [3  6  9]
		  /    |    \
	[1 2 3] [4 5 6] [7 8 9]
...
```
向已满节点插入新元素时，我们将该节点一分为二，新节点置于老节点之后。
```go
...
```go
	func (tr *Tree) Remove(key int) bool {
		...
		if unit.cnt <= LEAF_QUARTER {							//残量少，考虑合并
			var peer = unit
			...
			for combined {										//连锁
				...
				if unit.cnt <= INDEX_QUARTER {
					...
					combined = unit.combine(peer)				//尝试合并
					parent.data[place-1] = unit.ceil()			//修正索引
					continue
				}
				break
